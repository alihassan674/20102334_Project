...
export const getDashboardAnalytics = async (_req: Request, res: Response): Promise<void> => {
  ...
  const [
    totalHostels,      // Total number of hostel records
    ...
    prisma.hostel.count(),   // Simple count query
    prisma.room.count(),     // Simple count query
    ...
    prisma.student.count(), // Simple count query
  ...
  const monthlyRevenue = rooms.reduce(
    (sum: number, r: any) => sum + r._count.students * r.pricePerMonth, 0
  );
  ...
  const availableRooms = rooms.filter(
    (r: any) => r._count.students < r.capacity && r.status !== "MAINTENANCE"
  ).length;
